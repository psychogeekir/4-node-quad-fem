...
%%
% HollowArea: cell number of hollows x 1
fem2dinput.HollowArea{1,1} = (10-sqrt(3))*(5-sqrt(3)/2)*sqrt(3)/2;

% internalbcnode: cell conditions x number of hollows
internalbcnode{1,1} = find(abs(1/3*sqrt(3)/2*10-0.5-fem2dinput.x)<=1e-6 & fem2dinput.y>=-10/2+sqrt(3)*0.5-0.1 & fem2dinput.y<=10/2-sqrt(3)*0.5+0.1);
internalbcnode{2,1} = find(abs(tan(pi/6)*(fem2dinput.x+2/3*sqrt(3)/2*10-1)-fem2dinput.y)<=1e-6 & fem2dinput.y>=0-0.1 & fem2dinput.y<=10/2-sqrt(3)*0.5+0.1);
internalbcnode{3,1} = find(abs(-tan(pi/6)*(fem2dinput.x+2/3*sqrt(3)/2*10-1)-fem2dinput.y)<=1e-6 & fem2dinput.y<=0+0.1 & fem2dinput.y>=-10/2+sqrt(3)*0.5-0.1);

% InternalBCNode cell number of hollows x 1
fem2dinput.InternalBCNode{1,1} = unique(cell2mat(internalbcnode(1:end,1)));

%%
fem2dinput.MasterNode{1,1} = fem2dinput.InternalBCNode{1,1}(1);
fem2dinput.SlaveNode{1,1} = fem2dinput.InternalBCNode{1,1}(2:end);

fem2dinput.MasterDOF{1,1} = fem2dinput.MasterNode{1,1};
fem2dinput.SlaveDOF{1,1} = fem2dinput.SlaveNode{1,1};

%%
fem2dinput.nodnbc = fem2dinput.MasterDOF{1,1};
fem2dinput.valnbc = [2*G*fem2dinput.HollowArea{1,1}];
